...
static int open_input_file(const char *filename,
  ...
  if (!(file_iformat = av_find_input_format("f32le"))) {
    fprintf(stderr, "Unknown input format: 'f32le'\n");
    return -1;
  }

  /* Open the input file to read from it. */

  if ((error = avformat_open_input(input_format_context, filename, file_iformat,
                                   NULL)) < 0) {
...
int main(int argc, char **argv) {
  ...
  if (init_fifo(&fifo, output_codec_context)) goto cleanup;
  /* Write the header of the output file container. */
  if (write_output_file_header(output_format_context)) goto cleanup;
